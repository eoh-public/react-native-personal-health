...
import { PersonalHealthStack } from './PersonalHealthStack';
...
const NavStack = () => {
  return (
    ...
    >
      <Stack.Screen
        name={Routes.PersonalHealthStack}
        component={PersonalHealthStack}
        options={{
          headerShown: false,
        }}
      />
...
const App = ({ dataNotification, auth, onExitApp, langTranslate }) => {
  const exitApp = usePHSelector((state) => state.app.exitApp);
  const [loading, setLoading] = useState(true);
  const { setAction, setAuth } = useContext(PHContext);
